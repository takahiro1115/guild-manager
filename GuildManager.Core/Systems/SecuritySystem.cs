...
namespace GuildManager.Core.Systems
{
    ...
    public class SecuritySystem
    {
        ...
        /// <summary>
        /// 期限切れ（放置）になった討伐クエスト1件を脅威度上昇に反映する。討伐クエスト以外は
        /// 何もしない（その場合は0を返す）。実際に適用した増加量を返す（UI側の週報ログ表示用）。
        /// </summary>
        public int ApplyAbandonedQuest(GameState state, Quest quest)
        {
            ...
            int delta = _rng.NextInt(SecurityBalance.ThreatIncreaseMin, SecurityBalance.ThreatIncreaseMax);
            return ApplyDelta(state, delta);
        }
